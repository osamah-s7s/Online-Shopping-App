...
public class EditItemFragment extends Fragment implements FeaturedAdapter.OnItemClickListener {

    private RecyclerView editItemRecycler;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_edit__item, container, false);

        // Initialize database helper
        databaseHelper = new DatabaseHelper(requireContext());

        // Initialize RecyclerView
        editItemRecycler = view.findViewById(R.id.editItemRecycler);
        editItemRecycler.setHasFixedSize(true);
        editItemRecycler.setLayoutManager(new GridLayoutManager(requireContext(), 2));
    ...
    private void populateRecyclerView() {
        ...
        editItemRecycler.setAdapter(adapter);
    ...
    public void onItemClick(FeaturedHelperClass item) {
        ...
        EditDetailsFragment editDetailsFragment = EditDetailsFragment.newInstance(item.getImage(), item.getTitle(), item.getDescription());

        // Replace the current fragment with DetailsFragment
        requireActivity().getSupportFragmentManager().beginTransaction()
                .replace(R.id.fragmentContainer, editDetailsFragment)
                .addToBackStack(null)
                .commit();
    }
